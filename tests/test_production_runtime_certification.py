...
def test_runtime_certification_passes_with_build_info_when_expected_commit_unknown(tmp_path, monkeypatch):
    install_root = tmp_path / "opt" / "alfred"
    app = install_root / "app"
    output = app / "output"
    config_dir = install_root / "config"
...
"""
    ...
    report = build_runtime_certification(install_root)

    assert report.overall_status == "GREEN"
    statuses = {check.name: check.status for check in report.checks}
    assert statuses["Deployed commit matches expected commit"] == "PASS"
