"""Persistent project management state for Alfred."""
...
def management_store_path() -> Path:
    explicit = os.environ.get("ALFRED_PROJECT_MANAGEMENT_STORE")
    if explicit:
        return Path(explicit).expanduser().resolve()
    return management_data_dir() / "project_management_state.json"


def load_project_management_store() -> dict[str, Any]:
...
def save_project_management_store(store: dict[str, Any]) -> Path:
    path = management_store_path()
...
def get_project_record(store: dict[str, Any], project_id: str) -> dict[str, Any]:
...
def update_project_fields(project_id: str, changes: dict[str, Any], *, actor: str = "user", reason: str = "") -> dict[str, Any]:
    store = load_project_management_store()
    record = get_project_record(store, project_id)
    ...
    save_project_management_store(store)
    return record


def add_management_note(project_id: str, text: str, *, actor: str = "user", reason: str = "") -> dict[str, Any]:
    store = load_project_management_store()
    record = get_project_record(store, project_id)
    note = {
        "note_id": str(uuid4()),
...
def add_milestone(project_id: str, *, title: str, due_date: str | None = None, actor: str = "user", reason: str = "") -> dict[str, Any]:
    store = load_project_management_store()
    record = get_project_record(store, project_id)
    milestone = {
...
def complete_milestone(project_id: str, milestone_id: str, *, actor: str = "user", reason: str = "") -> dict[str, Any]:
    store = load_project_management_store()
    record = get_project_record(store, project_id)
...
def add_linked_item(project_id: str, field: str, item: dict[str, Any], *, current_items: list[dict[str, Any]] | None = None, actor: str = "user", reason: str = "") -> dict[str, Any]:
    store = load_project_management_store()
    record = get_project_record(store, project_id)
    values = record.setdefault("values", {})
...
def remove_linked_item(project_id: str, field: str, item_key: str, *, current_items: list[dict[str, Any]] | None = None, actor: str = "user", reason: str = "") -> dict[str, Any]:
    store = load_project_management_store()
    record = get_project_record(store, project_id)
    values = record.setdefault("values", {})
...
def set_project_status(project_id: str, status: str, rag_rating: str, *, actor: str = "user", reason: str = "") -> dict[str, Any]:
    return update_project_fields(
        project_id,
        {"current_status": status, "rag_rating": rag_rating},
        actor=actor,
        reason=reason or f"Project moved to {status}.",
    )


def merge_project_detail(detail: dict[str, Any], record: dict[str, Any] | None) -> dict[str, Any]:
